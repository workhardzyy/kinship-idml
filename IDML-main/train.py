...
fiw_data_path = '/home/yyh/zyy/IDML-main/sample0/train_sort_A2_m.txt'
fiw_data_val_path = '/home/yyh/zyy/IDML-main/sample0/val_A.txt'
trn_dataset = FIW2(fiw_data_path)
val_dataset = FIW2(fiw_data_val_path, mode ='val')
if args.IPC:
...
for epoch in range(0, args.nb_epochs):
    ...
    pbar_val = tqdm(enumerate(dl_ev))
    for batch_idx, data in enumerate(pbar):
        image1, image2,  labels, kin_label,class_ = data[1]
        image1, image2,  labels, kin_label = image1.float().cuda(), image2.float().cuda(),  labels.cuda(), kin_label.cuda()
        images = [image1,image2]
        mixed_images, y_1, y_2, lam = mixup(images, labels, 1.0)
        # m,v = model(img)
        m1, v1, m2, v2, pred_1 = model(images)
        mixed_m1, mixed_v1, mixed_m2,mixed_v2,pred_2 = model(mixed_images)
        loss_ori1 = criterion(m1, v1, y_1)
        loss_ori2 = criterion(m2, v2, y_1)
        loss_mixed1 = criterion(mixed_m1, mixed_v1, y_1)
        # loss_mixed2 = criterion(mixed_m1, mixed_v1, y_2)
        loss = loss_ori1*10000+loss_ori2*10000 + lam*loss_mixed1*10000
    ...
    if(epoch >= 0):
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(pbar_val):
                ...
                m1, v1, m2, v2, pred_1 = model(images)
                pred_1[pred_1<0] = 0
                pred_1[pred_1>thres] = 1
                pred_1 = pred_1.cpu().detach().numpy().tolist()
                labels = labels.cpu().detach().numpy().tolist()
                fpr, tpr, _ = roc_curve(labels, pred_1)
                auc_ = auc(fpr,tpr)
                print("auc: ", auc_)
    ...
    save_model_path = os.path.join(save_path,save_name)
